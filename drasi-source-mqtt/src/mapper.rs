...
///
/// Entity-centric: tracks previously seen IDs via `seen_ids`. A new ID produces
/// a `Create` operation; a known ID produces an `Update`.
///
/// # Arguments
...
/// * `seen_ids`  - Shared set tracking known entity IDs.
pub fn payload_to_source_change(
    ...
    // Determine operation: Create for new entities, Update for known ones.
    // insert() returns true if the value was NOT previously present → Create
    let is_create = seen_ids.insert(entity_id.clone());
...
mod tests {
    ...
    fn make_seen_ids() -> Arc<DashSet<String>> {
        Arc::new(DashSet::new())
    }

    #[test]
    fn test_create_on_first_see() {
        let seen = make_seen_ids();
        let payload = br#"{"id": "sensor-1", "temp": 25.5}"#;
        let change = payload_to_source_change(payload, "id", "Sensor", &seen).unwrap();
        ...
        assert!(seen.contains("sensor-1"));
    }

    #[test]
    fn test_update_on_second_see() {
        let seen = make_seen_ids();
        let payload = br#"{"id": "sensor-1", "temp": 25.5}"#;
        let _ = payload_to_source_change(payload, "id", "Sensor", &seen).unwrap();

        let payload2 = br#"{"id": "sensor-1", "temp": 30.0}"#;
        let change = payload_to_source_change(payload2, "id", "Sensor", &seen).unwrap();
    ...
    fn test_numeric_id_field() {
        ...
        let change = payload_to_source_change(payload, "device_id", "Sensor", &seen).unwrap();
    ...
    fn test_invalid_json() {
        ...
        assert!(payload_to_source_change(payload, "id", "Sensor", &seen).is_err());
    }
}
